...
void TestLogEntry::testLogLevelParsing()
{
    ...
    for (const QString& level : levels) {
        QString logLine =
            QString("[2025-06-27 08:36:19.123] [%1] [Module] : Message")
                .arg(level);
...
{
    QStringList modules = {"Core", "Network", "Database",
                           "UI",   "Parser",  "Exporter"};
...
{
    QStringList messages = {"Simple message",
                            "Message with 特殊字符 and symbols !@#$%",
                            "Very long message that spans multiple words and "
...
void TestLogEntry::testParsingPerformance()
{
    ...
    qDebug() << QString("Performance test: %1 entries processed in %2ms")
                    .arg(iterations)
                    .arg(elapsed);
