 ...
 * @details Provides comprehensive log export capabilities supporting TXT, CSV, and JSON
 *          formats with configurable field inclusion and batch export functionality.
 *          Includes progress tracking and error handling for large dataset exports.
 * @author GeziP
 * @date 2025-06-27
 * @version 1.0
 ...
 *          target formats, file paths, field inclusion settings, and naming conventions.
 *          Supports simultaneous export to multiple formats with a single configuration.
 * 
 * Key features:
...
struct ExportConfig {
    ...
    QString baseFileName;       ///< Base filename without extension (extension added automatically)
     ...
     *          log entry export by default. Format list and paths must be set explicitly.
     */
     ...
     * @return Complete file path including directory, base name, and format extension
 ...
 * @details This class provides comprehensive log export functionality supporting
 *          multiple file formats (TXT, CSV, JSON) with configurable field inclusion,
 *          progress tracking, and batch export capabilities. It inherits from QObject
 *          to provide signal-based progress updates and completion notifications.
 * 
 * Key features:
 ...
 * connect(&exporter, &LogExporter::progressChanged, this, &MyClass::updateProgress);
 * connect(&exporter, &LogExporter::exportFinished, this, &MyClass::handleExportComplete);
 * 
 * ExportConfig config;
...
public:
     ...
     * @details Exports the provided log entries using the first format specified
     *          in the configuration. Emits progress signals during export process.
     */
...
signals:
     ...
     *          providing final status and any relevant messages for user feedback.
     */
     ...
     *          allowing UI to provide detailed feedback about each exported file.
     */
...
private:
     ...
     *          appropriate for the specified format, respecting field inclusion settings.
     */
     ...
     * @details Handles comma, quote, and newline escaping according to CSV standards,
     *          ensuring data integrity in the exported CSV file.
     */
     ...
     * @details Handles quote, backslash, and control character escaping according
     *          to JSON standards, ensuring valid JSON output.
     */
     ...
     * @details Helper method that calculates percentage and emits progressChanged signal,
     *          providing consistent progress reporting across different export methods.
     */
