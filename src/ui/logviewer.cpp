...
LogViewer::LogViewer(QWidget* parent)
    : QMainWindow(parent), logModel(nullptr), currentSearchIndex(-1)
{
    setupUI();
...
void LogViewer::loadLogFile(const QString& filePath)
{
    ...
    statusBar()->showMessage(
        tr("已加载文件：%1，日志条目数：%2").arg(filePath).arg(allLogs.size()));
}
...
void LogViewer::onFilterButtonClicked()
{
    ...
    QList<LogEntry> filteredLogs = filterLogs(allLogs, startTime, endTime,
    ...
    statusBar()->showMessage(
        tr("筛选完成，日志条目数：%1").arg(filteredLogs.size()));
}
...
void LogViewer::displayLogs(const QList<LogEntry>& logs)
{
    ...
    QStringList headers = {tr("行号"), tr("时间"), tr("等级"), tr("模块"),
                           tr("内容")};
    ...
    for (int i = 0; i < logModel->columnCount(); ++i) {
        logTreeView->header()->setSectionResizeMode(i,
    ...
    logTreeView->header()->resizeSection(
        0, 80); // Increase line number column width to 80 pixels
}
...
void LogViewer::searchInItem(QStandardItem* item)
{
    ...
    if (!contentItem) {
        contentItem =
            item->model()->itemFromIndex(item->index().sibling(item->row(), 4));
    }
    if (contentItem &&
        contentItem->text().contains(currentSearchText, Qt::CaseInsensitive)) {
        matched = true;
...
void LogViewer::clearHighlightsInItem(QStandardItem* item)
{
    ...
    for (int col = 0; col < item->columnCount(); ++col) {
        ...
        if (!cellItem) {
            cellItem = item->model()->itemFromIndex(
                item->index().sibling(item->row(), col));
        }
        if (cellItem) {
...
void LogViewer::onLogItemDoubleClicked(const QModelIndex& index)
{
    ...
    for (int col = 0; col < logModel->columnCount(); ++col) {
        ...
        QString data =
            logModel->data(sourceIndex.sibling(sourceIndex.row(), col))
                .toString();
        details += QString("%1: %2\n").arg(header).arg(data);
    }
...
QList<LogEntry> LogViewer::getCurrentFilteredLogs() const
{
    ...
    for (int i = 0; i < logModel->rowCount(); ++i) {
        ...
        if (timestampItem && levelItem && moduleItem && contentItem) {
            entry.timestamp = QDateTime::fromString(timestampItem->text(),
                                                    "yyyy-MM-dd HH:mm:ss.zzz");
...
void LogViewer::retranslateUI()
{
    ...
    if (logModel) {
        QStringList headers;
        headers << tr("行号") << tr("时间") << tr("等级") << tr("模块")
                << tr("内容");
        logModel->setHorizontalHeaderLabels(headers);
