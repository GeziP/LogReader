...
ExportDialog::ExportDialog(QWidget* parent) : QDialog(parent), logCount(0)
{
...
void ExportDialog::setupUI()
{
    ...
    filePathLineEdit->setPlaceholderText(
        tr("选择导出文件夹，文件名自动生成..."));
    ...
    okButton->setStyleSheet("background-color: #4CAF50; color: white; padding: "
                            "8px 16px; border-radius: 4px; font-weight: bold;");
...
{
    connect(txtCheckBox, &QCheckBox::toggled, this,
            &ExportDialog::onFormatChanged);
    connect(csvCheckBox, &QCheckBox::toggled, this,
            &ExportDialog::onFormatChanged);
    connect(jsonCheckBox, &QCheckBox::toggled, this,
            &ExportDialog::onFormatChanged);

    connect(timestampCheckBox, &QCheckBox::toggled, this,
            &ExportDialog::updatePreview);
    connect(levelCheckBox, &QCheckBox::toggled, this,
            &ExportDialog::updatePreview);
    connect(moduleCheckBox, &QCheckBox::toggled, this,
            &ExportDialog::updatePreview);
    connect(contentCheckBox, &QCheckBox::toggled, this,
            &ExportDialog::updatePreview);

    connect(browseButton, &QPushButton::clicked, this,
            &ExportDialog::onBrowseFileClicked);
...
void ExportDialog::onBrowseFileClicked()
{
    ...
    QString directoryPath =
        QFileDialog::getExistingDirectory(this, tr("选择导出文件夹"),
                                          defaultPath);
...
void ExportDialog::updatePreview()
{
    ...
    if (txtCheckBox->isChecked()) {
        ...
        if (timestampCheckBox->isChecked())
            parts << sampleTime.toString("yyyy-MM-dd hh:mm:ss");
    ...
    if (jsonCheckBox->isChecked()) {
        QStringList fields;
        if (timestampCheckBox->isChecked())
            fields << QString("\"timestamp\": \"%1\"")
                          .arg(sampleTime.toString(Qt::ISODate));
        if (levelCheckBox->isChecked())
            fields << "\"level\": \"INFO\"";
        if (moduleCheckBox->isChecked())
            fields << "\"module\": \"系统\"";
    ...
    QString preview = previews.isEmpty() ? tr("请选择至少一种导出格式")
                                         : previews.join("\\n\\n");
...
{
    // 验证格式选择
    if (!txtCheckBox->isChecked() && !csvCheckBox->isChecked() &&
        !jsonCheckBox->isChecked()) {
...
void ExportDialog::loadSettings()
{
    ...
    AppSettings::instance().getLastIncludeFields(timestamp, level, module,
                                                 content);
...
void ExportDialog::saveSettings()
{
    ...
    // 保存字段选择
    AppSettings::instance().setLastIncludeFields(timestampCheckBox->isChecked(),
